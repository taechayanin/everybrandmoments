...
describe("Zero-activity no-contact risk (Step-5 fix 2)", () => {
  ...
  it("canonical rule covers all five reviewer cases", async () => {
    ...
    expect(isOpportunityAtRisk("ACTIVE", null, daysAgo(2), NOW)).toBe(false);
    // zero activity + 10 days -> at risk (falls back to created_at)
    expect(isOpportunityAtRisk("ACTIVE", null, daysAgo(10), NOW)).toBe(true);
    // activity yesterday -> not at risk
    expect(isOpportunityAtRisk("ACTIVE", daysAgo(1), daysAgo(30), NOW)).toBe(false);
    // activity 8 days ago -> at risk
    expect(isOpportunityAtRisk("ACTIVE", daysAgo(8), daysAgo(30), NOW)).toBe(true);
    ...
    expect(isOpportunityAtRisk("ACTIVE", daysAgo(7), daysAgo(30), NOW)).toBe(true);
  ...
  it("counter and per-row display share the rule through the queue view", async () => {
    const { opportunity: won } = await repos.opportunities.create({
    ...
    const zeroActivityOld = view.rows.find(
      (r) => r.lastActivityAt === null && r.opportunity.salesStage === "SOLUTION_DESIGN",
    );
