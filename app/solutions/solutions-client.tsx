...
import { Card, MomentChip } from "@/components/ui";
import { baht, pct } from "@/lib/format";
import type { MomentCode, Stakeholder } from "@/lib/types";
import type { SolutionWithNames } from "@/lib/application/solutions/list-solutions";
        ...
        {filtered.map((s) => (
          <Card key={s.id} className="flex h-full flex-col p-4">
            ...
            <div className="mt-auto pt-3">
              {s.crossSellNames.length > 0 && (
                <p className="text-[11px] text-slate-500">
                  <span className="font-semibold">Cross-sell:</span> {s.crossSellNames.join(" · ")}
                </p>
              )}
